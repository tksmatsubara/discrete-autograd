...
def train(args):
    ...
    for i in range(trials):
        ...
        true_x, model_x = integrate_models(x0=x0, t_span=t_span, t_eval=t_eval, noise_std=args.noise)
    ...
    energy_mse_mean = np.mean(losses['model_energy'])
    print("energy MSE {:.4e}".format(energy_mse_mean))
    stats['energy_mse_mean'] = energy_mse_mean
    state_mse_mean = np.mean(losses['model_state'])
    print("state MSE {:.4e}".format(state_mse_mean))
    stats['state_mse_mean'] = state_mse_mean
